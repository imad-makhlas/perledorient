export { AdminDashboardPage as AdminProductsPage } from './AdminDashboardPage'

/* Legacy implementation retained temporarily for an easy local comparison.
...
export function AdminProductsPage() {
  ...
  const save = async (product: AdminProduct) => {
    ...
    try { const saved = await updateAdminProduct(credentials, product); setProducts((current) => current.map((item) => item.id === saved.id ? saved : item)); setMessage(`${saved.nameEn} saved.`) }
  ...
  return <main className="container-shell py-12 lg:py-16">
    ...
    {!products.length ? <div className="mt-14 grid place-items-center py-16 text-center text-muted"><Package size={34} strokeWidth={1.2} /><p className="mt-4 text-sm">Sign in to manage your pieces.</p></div> : <div className="mt-8 grid gap-6">{products.map((product) => <article key={product.id} className="grid gap-6 border border-line bg-white p-5 shadow-soft lg:grid-cols-[170px_1fr_auto]">
      ...
      <div className="grid gap-4 sm:grid-cols-2"><label className="text-[10px] font-bold uppercase tracking-widest">Piece name<input className="field mt-2 normal-case tracking-normal" value={product.nameEn} onChange={(event) => edit(product.id, 'nameEn', event.target.value)} /></label><label className="text-[10px] font-bold uppercase tracking-widest">Finish<input className="field mt-2 normal-case tracking-normal" value={product.variantName} onChange={(event) => edit(product.id, 'variantName', event.target.value)} /></label><label className="text-[10px] font-bold uppercase tracking-widest">Price (MAD)<input className="field mt-2 normal-case tracking-normal" type="number" min="0" value={product.price} onChange={(event) => edit(product.id, 'price', Number(event.target.value))} /></label><label className="text-[10px] font-bold uppercase tracking-widest">Stock<input className="field mt-2 normal-case tracking-normal" type="number" min="0" value={product.stock} onChange={(event) => edit(product.id, 'stock', Number(event.target.value))} /></label><label className="text-[10px] font-bold uppercase tracking-widest sm:col-span-2">Photo URL<input className="field mt-2 normal-case tracking-normal" value={product.imageUrl ?? ''} onChange={(event) => edit(product.id, 'imageUrl', event.target.value)} placeholder="Paste your product image URL" /></label><label className="flex items-center gap-3 text-xs font-semibold sm:col-span-2"><input type="checkbox" checked={product.active} onChange={(event) => edit(product.id, 'active', event.target.checked)} />Available in the shop</label></div>

...
export function AdminDashboardPage() {
  ...
  const removeProduct = async (product: AdminProduct) => {
    ...
    try {
      ...
      try { await Promise.all(product.imageUrls.map((imageUrl) => deleteAdminImage(credentials, imageUrl))); setMessage('Bijou et photos supprimés.') }
  ...
  return <main className="min-h-screen bg-[#F7F4EF] text-[#302A2E]">
    <header className="sticky top-0 z-30 border-b border-white/10 bg-[#302A2E] text-white shadow-lg"><div className="mx-auto flex max-w-[1500px] items-center gap-4 px-4 py-3 sm:px-7"><button onClick={() => window.location.assign('/')} className="grid h-10 w-10 place-items-center rounded-full border border-white/15"><ArrowLeft size={17} /></button><div><p className="text-[9px] font-bold uppercase tracking-[.22em] text-[#C4943D]">Casa de Perla</p><p className="display text-xl font-semibold">Atelier</p></div><button onClick={() => { setCredentials(null); setProducts([]); setOrders([]) }} className="ml-auto inline-flex items-center gap-2 rounded-full border border-white/15 px-4 py-2 text-[10px] font-bold uppercase tracking-wider"><LogOut size={14} />Déconnexion</button></div></header>
    <div className="mx-auto grid max-w-[1500px] lg:grid-cols-[250px_1fr]">
      <aside className="border-b border-[#DDD4C9] bg-white p-2 lg:min-h-[calc(100vh-65px)] lg:border-b-0 lg:border-r lg:p-5"><nav className="grid grid-cols-4 gap-1 lg:flex lg:flex-col lg:gap-2">{navigation.map(([key, label, Icon]) => <button key={key} onClick={() => setView(key)} className={`relative flex min-w-0 flex-col items-center justify-center gap-1 rounded-[6px] px-1 py-2.5 text-[9px] font-bold uppercase tracking-[.08em] transition lg:flex-row lg:justify-start lg:gap-3 lg:px-4 lg:py-3 lg:text-left lg:text-xs lg:tracking-wider ${view === key ? 'bg-[#302A2E] text-white' : 'text-[#6D6266] hover:bg-[#F7F4EF]'}`}><Icon size={17} className={view === key ? 'text-[#C4943D]' : ''} /><span className="truncate">{label}</span>{key === 'orders' && pendingOrders > 0 && <span className="absolute right-2 top-1 rounded-full bg-[#C4943D] px-1.5 py-0.5 text-[8px] text-[#302A2E] lg:static lg:ml-auto lg:px-2 lg:text-[9px]">{pendingOrders}</span>}</button>)}</nav></aside>
      <section className="min-w-0 p-4 sm:p-7 lg:p-10">
        {message && <div className="mb-6 flex items-center justify-between rounded-[6px] border border-[#DCC8A1] bg-white px-4 py-3 text-sm"><span>{message}</span><button onClick={() => setMessage('')}><X size={16} /></button></div>}
...
function Overview({ products, orders, onAdd, onOrders }: { products: AdminProduct[]; orders: AdminOrder[]; onAdd: () => void; onOrders: () => void }) {
  ...
  return <div><p className="eyebrow">Aujourd’hui</p><h1 className="display mt-2 text-4xl font-semibold sm:text-5xl">Vue d’ensemble</h1><div className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{stats.map(([label, value, Icon]) => <article key={label} className="rounded-[6px] border border-[#DDD4C9] bg-white p-5 shadow-[0_12px_35px_rgba(48,42,46,.05)]"><Icon size={20} className="text-[#C4943D]" /><p className="mt-6 text-3xl font-semibold">{value}</p><p className="mt-1 text-xs uppercase tracking-wider text-[#7B7074]">{label}</p></article>)}</div><div className="mt-7 grid gap-4 lg:grid-cols-2"><button onClick={onAdd} className="rounded-[6px] bg-[#302A2E] p-6 text-left text-white"><Plus className="text-[#C4943D]" /><p className="display mt-5 text-2xl font-semibold">Ajouter un nouveau bijou</p><p className="mt-2 text-sm text-white/55">Publiez une pièce en français et en arabe.</p></button><button onClick={onOrders} className="rounded-[6px] border border-[#DDD4C9] bg-white p-6 text-left"><ShoppingBag className="text-[#C4943D]" /><p className="display mt-5 text-2xl font-semibold">Voir les commandes</p><p className="mt-2 text-sm text-[#7B7074]">Confirmez et suivez les demandes WhatsApp.</p></button></div></div>
}
...
function Catalogue({ products, total, query, setQuery, onAdd, onEdit, onDelete }: { products: AdminProduct[]; total: number; query: string; setQuery: (value: string) => void; onAdd: () => void; onEdit: (product: AdminProduct) => void; onDelete: (product: AdminProduct) => void }) {
  const itemLabel = total > 1 ? 'bijoux enregistrés' : 'bijou enregistré'
  return <div><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="eyebrow">Gestion des pièces</p><h1 className="display mt-2 text-4xl font-semibold sm:text-5xl">Catalogue</h1><p className="mt-2 text-sm text-[#7B7074]">{total} {itemLabel}</p></div><button onClick={onAdd} className="button-primary button-accent"><Plus size={16} />Ajouter un bijou</button></div><label className="mt-7 flex min-h-12 items-center gap-3 rounded-[6px] border border-[#DDD4C9] bg-white px-4"><Search size={17} /><input className="w-full bg-transparent text-sm outline-none" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Rechercher par nom ou référence…" /></label><div className="mt-5 grid gap-4">{products.map((product) => <article key={product.id} className="grid gap-4 rounded-[6px] border border-[#DDD4C9] bg-white p-4 shadow-[0_10px_30px_rgba(48,42,46,.04)] sm:grid-cols-[100px_1fr_auto] sm:items-center"><div className="aspect-square overflow-hidden rounded-[6px] bg-[#F2EEE8]">{product.imageUrl && <img src={product.imageUrl} alt="" className="h-full w-full object-cover" />}</div><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h2 className="display truncate text-xl font-semibold">{product.nameFr}</h2><span className={`rounded-full px-2.5 py-1 text-[9px] font-bold uppercase tracking-wider ${product.active ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-600'}`}>{product.active ? 'Publié' : 'Masqué'}</span></div><p className="mt-1 text-xs text-[#7B7074]">{product.nameAr} · {product.sku}</p><div className="mt-3 flex flex-wrap gap-4 text-sm"><strong>{formatMoney(product.price)}</strong><span>Stock : {product.stock}</span><span>{categoryLabel(product.category as CatalogCategory, 'fr')}</span></div></div><div className="flex gap-2 sm:justify-end"><button onClick={() => onEdit(product)} className="grid h-11 w-11 place-items-center rounded-[6px] border border-[#DDD4C9]" aria-label="Modifier"><Edit3 size={16} /></button><button onClick={() => onDelete(product)} className="grid h-11 w-11 place-items-center rounded-[6px] border border-red-200 text-red-700" aria-label="Supprimer"><Trash2 size={16} /></button></div></article>)}</div></div>
}
...
function Orders({ orders, busy, onUpdate, onDelete }: { orders: AdminOrder[]; busy: boolean; onUpdate: (order: AdminOrder, status: AdminOrderStatus) => void; onDelete: (order: AdminOrder) => void }) {
  ...
  return <div>
    ...
    <div className="mt-7 grid gap-5">
      {orders.length === 0 && <div className="rounded-[6px] border border-[#DDD4C9] bg-white p-12 text-center text-sm text-[#7B7074] shadow-[0_12px_38px_rgba(48,42,46,.04)]">Aucune commande pour le moment.</div>}
      {orders.map((order) => <article key={order.orderNumber} className="overflow-hidden rounded-[6px] border border-[#DDD4C9] bg-white shadow-[0_16px_45px_rgba(48,42,46,.055)]">
        ...
        <div className="grid gap-6 p-5 sm:p-7 lg:grid-cols-[minmax(240px,.75fr)_minmax(0,1.25fr)] lg:gap-8">
          <section role="group" aria-label="Coordonnées client" className="min-w-0 rounded-[6px] border border-[#E7DED4] bg-[#FCFAF7] p-4 sm:p-5">
            <p className="text-[9px] font-bold uppercase tracking-[.16em] text-[#A06F22]">Coordonnées client</p>
            <div className="mt-4 space-y-4 text-sm">
              ...
              <div className="flex min-w-0 gap-3"><Phone size={17} className="mt-0.5 shrink-0 text-[#C4943D]" /><div className="min-w-0"><p className="text-[9px] uppercase tracking-wider text-[#8A7E82]">Téléphone</p><a href={`tel:${order.customerTelephone}`} className="mt-1 block break-all font-semibold hover:text-[#A06F22]">{order.customerTelephone}</a></div></div>
        ...
        <div role="group" aria-label="Actions de la commande" className="flex flex-col gap-2 border-t border-[#E7DED4] bg-[#FCFAF7] px-5 py-4 sm:flex-row sm:flex-wrap sm:px-7">
          {order.whatsappUrl && <a href={order.whatsappUrl} target="_blank" rel="noreferrer" className="inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-[6px] bg-[#C4943D] px-5 text-[10px] font-bold uppercase tracking-[.12em] text-[#241F21] transition hover:bg-[#D2A852] sm:w-auto"><MessageCircle size={16} />Contacter sur WhatsApp</a>}
          <button type="button" onClick={() => setPrintOrder(order)} className="inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-[6px] border border-[#D6C9BC] bg-white px-5 text-[10px] font-bold uppercase tracking-[.12em] text-[#302A2E] transition hover:border-[#C4943D] hover:bg-[#FBF7F0] sm:w-auto"><Printer size={15} />Imprimer le ticket</button>
          {getNextAdminActions(order.status).map((status) => <button key={status} disabled={busy} onClick={() => onUpdate(order, status)} className={`inline-flex min-h-12 w-full items-center justify-center rounded-[6px] border px-5 text-[10px] font-bold uppercase tracking-[.12em] transition disabled:opacity-50 sm:w-auto ${orderStatusPalette(status).action}`}>{orderStatusLabel(status, 'fr')}</button>)}
          <button disabled={busy} onClick={() => onDelete(order)} className="inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-[6px] border border-[#E3C2C2] bg-white px-5 text-[10px] font-bold uppercase tracking-[.12em] text-[#8A3D3D] transition hover:bg-[#FBF0F0] disabled:opacity-50 sm:ml-auto sm:w-auto"><Trash2 size={15} />Supprimer</button>
        </div>
      </article>)}
    </div>
    {printOrder && <OrderPrintTicket order={printOrder} onClose={() => setPrintOrder(null)} />}
  </div>
}

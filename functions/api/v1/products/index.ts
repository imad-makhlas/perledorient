...
import { attachProductImages, json, productColumns, type PagesContext } from '../admin/_shared'

export async function onRequestGet({ request, env }: PagesContext) {
  const locale = new URL(request.url).searchParams.get('locale') === 'ar' ? 'ar' : 'fr'
  ...
  return json((await attachProductImages(env.DB, result.results || [])).map((row) => catalogProductFromRow(row, locale)))
}

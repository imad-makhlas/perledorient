...
export function ProductImageViewer({ src, alt, openLabel, closeLabel, dialogLabel, enlargedAlt, hint }: ProductImageViewerProps) {
  ...
  return <>
    ...
    >
      <div onMouseDown={(event) => event.stopPropagation()} className="relative inline-flex max-h-[calc(100vh-2rem)] items-center justify-center overflow-hidden rounded-[6px] border border-white/50 bg-[#F8F5F0] p-2 shadow-[0_30px_90px_rgba(24,19,21,.32)] sm:max-h-[calc(100vh-4rem)] sm:rounded-[6px] sm:p-3">
        <img src={src} alt={enlargedAlt} className="block h-auto max-h-[calc(100vh-3rem)] w-auto max-w-[calc(100vw-3rem)] rounded-[6px] object-contain sm:max-h-[calc(100vh-5.5rem)] sm:max-w-[calc(100vw-5.5rem)] sm:rounded-[6px]" />

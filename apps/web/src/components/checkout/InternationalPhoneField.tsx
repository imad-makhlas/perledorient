...
export function InternationalPhoneField({ country, value, locale, label, error, onCountryChange, onChange }: InternationalPhoneFieldProps) {
  ...
  return <div className="min-w-0 text-[10px] font-bold uppercase tracking-widest">
    ...
    <div className="mt-2 grid min-w-0 grid-cols-[104px_minmax(0,1fr)] gap-2" dir="ltr">
      <div className="relative h-14 overflow-hidden rounded-2xl border border-[#D9CEC1] bg-[#F7F2EA] shadow-[inset_0_1px_0_rgba(255,255,255,.7)] transition focus-within:border-[#C4943D] focus-within:ring-2 focus-within:ring-[#C4943D]/10">
      ...
      <input id="checkout-telephone" className="h-14 w-full min-w-0 max-w-full rounded-2xl border border-[#D9CEC1] bg-white px-4 text-base font-normal normal-case tracking-normal outline-none transition placeholder:text-[#A69B9E] focus:border-[#C4943D] focus:ring-2 focus:ring-[#C4943D]/10" type="tel" inputMode="tel" autoComplete="tel-national" value={value} onChange={(event) => onChange(event.target.value)} aria-invalid={Boolean(error)} placeholder={country === 'MA' ? '06 31 21 06 54' : undefined} />
    </div>

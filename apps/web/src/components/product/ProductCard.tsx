...
export function ProductCard({ product }: { product: Product }) {
  ...
  return <article className="group h-full min-w-0 overflow-hidden rounded-[22px] border border-line bg-white p-2.5 shadow-[0_10px_32px_rgba(47,42,44,0.045)] transition duration-500 hover:-translate-y-1 hover:border-champagne hover:shadow-[0_20px_50px_rgba(47,42,44,0.10)] sm:p-3">
    <Link to={`/products/${product.slug}`} className="relative block aspect-square overflow-hidden rounded-[16px] bg-[#F3F1ED]">
    ...
    <div className="flex min-h-[178px] flex-col px-2 pb-1 pt-4 sm:px-2 sm:pb-1 sm:pt-4">
      ...
      {product.dimensions && <div className="mt-3 flex items-center gap-2 text-[12px] leading-snug text-muted"><Ruler size={14} strokeWidth={1.5} className="shrink-0 text-accent" /><span><strong className="font-semibold text-ink/70">{locale === 'fr' ? 'Taille' : 'Size'}</strong><span className="mx-1.5 text-accent">·</span>{product.dimensions}</span></div>}
      ...
      <button type="button" onClick={orderDirectly} disabled={product.stock <= 0} aria-label={`Order ${product.name} via WhatsApp`} className="mt-3 inline-flex min-h-[44px] w-full items-center justify-center gap-2 rounded-xl bg-[#2F2A2C] px-3 text-[9px] font-bold uppercase tracking-[.1em] text-[#FFF9F0] transition-colors hover:bg-[#40393C] disabled:cursor-not-allowed disabled:bg-[#E8E1D9] disabled:text-[#8B7E80]"><MessageCircle size={15} className="text-accent" aria-hidden="true" />{locale === 'fr' ? 'Commander via WhatsApp' : 'Order via WhatsApp'}</button>
    </div>
  </article>
}

...
const defaults: CheckoutForm = {
  firstName: '', lastName: '', telephone: '', email: '', city: 'Casablanca',
  address: '', postalCode: '', deliveryNotes: '', paymentMethod: 'WHATSAPP', acceptedTerms: true,
}
...
export function WhatsAppCheckoutPage() {
  ...
  const fields: [keyof CheckoutForm, string, string][] = [
    ...
    ['telephone', t('telephone'), 'tel'], ['email', t('email'), 'email'],
    ['city', t('city'), 'text'], ['postalCode', 'Postal code (optional)', 'text'],
  ]

  return <main className="container-shell py-12 lg:py-16">
    <div className="mb-10 text-center"><p className="eyebrow">WhatsApp order</p><h1 className="display mt-2 text-5xl font-semibold">{t('checkoutTitle')}</h1><p className="mx-auto mt-4 max-w-xl text-sm leading-7 text-muted">Share your delivery details, then continue in WhatsApp to confirm availability directly with the artisan.</p></div>
    <form onSubmit={submit} className="grid gap-8 lg:grid-cols-[1fr_420px]">
      <section className="border border-line bg-white p-6 shadow-soft sm:p-9">
        ...
        <div className="mt-7 flex items-start gap-4 border border-champagne bg-canvas p-5"><MessageCircle className="mt-0.5 shrink-0 text-accent" size={20} /><div><p className="text-xs font-bold uppercase tracking-widest">Direct WhatsApp confirmation</p><p className="mt-2 text-xs leading-5 text-muted">No online payment and no customer account. We confirm your piece, delivery and payment details personally on WhatsApp.</p></div></div>
      ...
      <aside className="h-fit bg-burgundy p-7 text-white lg:sticky lg:top-28">

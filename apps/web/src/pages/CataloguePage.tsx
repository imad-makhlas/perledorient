...
export function CataloguePage() {
  ...
  const results = useMemo(() => filterProducts(products, { search, category, inStockOnly: false, sort }), [products, search, category, sort])
  const ui = catalogUi(locale)
  const categoryValues = ['', ...categories.map((item) => item.name).filter((value) => value === 'Necklaces' || value === 'Bracelets')]
  ...
  return <main className="min-h-screen bg-white">
    <div className="container-shell py-4 lg:py-6">
      <section className="catalog-toolbar" aria-label={locale === 'fr' ? 'Outils du catalogue' : 'أدوات الكتالوج'}>
        ...
        <div className="catalog-filter-row catalog-filter-row-compact">
        ...
        {(search || category) && <button type="button" onClick={() => { setSearch(''); setCategory('') }} className="catalog-clear">{locale === 'fr' ? 'Effacer les filtres' : 'مسح التصفية'}</button>}
      </section>

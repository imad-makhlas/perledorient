...
export function ProductCard({ product }: { product: Product }) {
  ...
  return <article className="group h-full min-w-0 overflow-hidden rounded-[22px] border border-line bg-white p-2.5 shadow-[0_10px_32px_rgba(47,42,44,0.045)] transition duration-500 hover:-translate-y-1 hover:border-champagne hover:shadow-[0_20px_50px_rgba(47,42,44,0.10)] sm:p-3">
    ...
    <div className="flex min-h-[178px] flex-col px-2 pb-1 pt-4 sm:px-2 sm:pb-1 sm:pt-4">
      ...
      <button type="button" onClick={orderDirectly} disabled={product.stock <= 0} aria-label={locale === 'fr' ? `Commander ${product.name} via WhatsApp` : `طلب ${product.name} عبر واتساب`} className="mt-3 inline-flex min-h-[44px] w-full items-center justify-center gap-2 rounded-xl bg-[#2F2A2C] px-3 text-[9px] font-bold uppercase tracking-[.1em] text-[#FFF9F0] transition-colors hover:bg-[#40393C] disabled:cursor-not-allowed disabled:bg-[#E8E1D9] disabled:text-[#8B7E80]"><MessageCircle size={15} className="text-accent" aria-hidden="true" />{locale === 'fr' ? 'Commander via WhatsApp' : 'الطلب عبر واتساب'}</button>
    </div>
  </article>
}

...
export function ProductEditor({ product, suggestedSku, onClose, onSave, onUploadImage, onDeleteImage, busy }: ProductEditorProps) {
  ...
  return <div className="fixed inset-0 z-50 bg-black/35 p-0 backdrop-blur-sm sm:p-5" role="dialog" aria-modal="true">
    <form onSubmit={submit} className="ml-auto flex h-full w-full max-w-3xl flex-col overflow-hidden bg-white shadow-2xl sm:rounded-[6px]">
      ...
      <div className="flex-1 space-y-7 overflow-y-auto p-5 sm:p-7">
        ...
        <section><p className="mb-4 text-[10px] font-bold uppercase tracking-[.18em] text-accent">Informations produit</p><div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="text-xs font-semibold"><p>Référence produit (SKU)</p><div className="mt-2 rounded-[6px] border border-[#DDD4C9] bg-[#F8F5F0] px-4 py-3"><strong className="tracking-[.08em]">{product?.sku || suggestedSku}</strong><span className="mt-1 block text-[10px] font-normal text-[#7B7074]">{product ? 'Référence produit permanente' : 'Prochaine référence, confirmée à l’enregistrement'}</span></div></div>
        ...
        <section>
          ...
          {imageError && <p role="alert" className="mt-3 rounded-[6px] border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{imageError}</p>}
        </section>
        <div className="flex flex-wrap gap-5"><label className="flex items-center gap-3 text-sm font-semibold"><input type="checkbox" checked={draft.active} onChange={(event) => set('active', event.target.checked)} />Publié dans la boutique</label><div><label className="flex items-center gap-3 text-sm font-semibold"><input type="checkbox" checked={draft.featured} onChange={(event) => set('featured', event.target.checked)} />Mettre en avant</label><p className="mt-1.5 max-w-sm text-[11px] leading-5 text-[#7B7074]">Plusieurs bijoux peuvent être mis en avant en même temps. Ils apparaissent en priorité sur l’accueil.</p></div></div>
      </div>

...
import { DeliveryCityField } from '../components/checkout/DeliveryCityField'
...
import { getMoroccanCitySuggestions } from '../features/checkout/moroccan-cities'
...
export function WhatsAppCheckoutPage() {
  ...
  const citySuggestions = getMoroccanCitySuggestions(deliverySettings)
  ...
  return <main className="bg-white py-8 lg:py-12">
    <div className="container-shell">
      ...
      <form onSubmit={submit} className="grid min-w-0 items-start gap-6 lg:grid-cols-[minmax(0,1fr)_400px] xl:gap-8">
        <section className="min-w-0 overflow-hidden rounded-[6px] border border-[#E2D8CD] bg-[#FFFEFC] shadow-[0_20px_60px_rgba(48,42,46,.07)]">
          ...
          <div className="border-t border-[#E8DFD5] bg-[#FCF9F5] p-5 sm:p-8">
            ...
            <div className="mt-6 grid gap-5"><DeliveryCityField value={form.city} country={form.country || 'MA'} locale={locale} label={t('city')} cities={citySuggestions} error={errors.city} onChange={(value) => set('city', value)} /></div>
            <label className="mt-5 block text-[10px] font-bold uppercase tracking-[.14em] text-[#51474B]">{t('address')}<input className="mt-2 h-14 w-full rounded-[6px] border border-[#D9CEC1] bg-white px-4 text-base font-normal normal-case tracking-normal outline-none transition focus:border-[#C4943D] focus:ring-2 focus:ring-[#C4943D]/10" value={form.address} onChange={(event) => set('address', event.target.value)} />{errors.address && <span className="mt-1.5 block text-[11px] normal-case tracking-normal text-red-700">{errors.address}</span>}</label>
            <label className="mt-5 block text-[10px] font-bold uppercase tracking-[.14em] text-[#51474B]">{t('notes')}<textarea className="mt-2 min-h-24 w-full resize-y rounded-[6px] border border-[#D9CEC1] bg-white px-4 py-4 text-base font-normal normal-case tracking-normal outline-none transition focus:border-[#C4943D] focus:ring-2 focus:ring-[#C4943D]/10" value={form.deliveryNotes} onChange={(event) => set('deliveryNotes', event.target.value)} /></label>
          </div>
          <div className="border-t border-[#E8DFD5] p-5 sm:p-8"><div className="flex items-start gap-4 rounded-[6px] border border-[#DFC99F] bg-[#FBF6EC] p-4 sm:p-5"><MessageCircle className="mt-0.5 shrink-0 text-[#B37A22]" size={20} /><div><p className="text-xs font-bold uppercase tracking-[.14em]">{locale === 'fr' ? 'Confirmation personnelle' : 'تأكيد شخصي'}</p><p className="mt-2 text-xs leading-6 text-muted">{locale === 'fr' ? "Aucun paiement en ligne. Votre pièce, la livraison et les modalités sont confirmées avec vous sur WhatsApp." : 'لا يوجد دفع إلكتروني. نؤكد معك عبر واتساب توفر القطعة والتوصيل وتفاصيل الطلب.'}</p></div></div><label className="mt-5 flex items-start gap-3 text-xs leading-5 text-muted"><input className="mt-0.5 h-4 w-4 accent-[#B9822D]" type="checkbox" checked={form.acceptedTerms} onChange={(event) => set('acceptedTerms', event.target.checked)} />{t('consent')}</label>{errors.acceptedTerms && <p className="mt-2 text-xs text-red-700">{locale === 'fr' ? 'Veuillez accepter les conditions.' : 'يرجى قبول الشروط.'}</p>}{errors.form && <p className="mt-4 rounded-[6px] bg-red-50 p-3 text-sm text-red-700">{errors.form}</p>}</div>
        </section>
        <aside className="h-fit overflow-hidden rounded-[6px] border border-[#DED4C8] bg-white shadow-[0_20px_60px_rgba(48,42,46,.08)] lg:sticky lg:top-28">

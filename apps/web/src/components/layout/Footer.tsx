...
export function Footer() {
  ...
  return <footer className="border-t-2 border-accent bg-[#2F2A2C] text-white">
    <div className="container-shell grid grid-cols-2 gap-x-5 gap-y-8 py-10 md:grid-cols-[1.45fr_1fr_1fr_1.2fr] md:gap-10 md:py-14 lg:py-16">
      ...
      <div role="region" aria-label="Coordonnées Casa de Perla" dir="ltr" className="col-span-2 border-t border-white/10 pt-6 text-left md:col-span-1 md:border-0 md:pt-0"><h3 className="text-[10px] font-bold uppercase tracking-[.2em]">Casa de Perla</h3><div className="mt-4 flex flex-wrap gap-x-6 gap-y-3 text-[13px] text-white/55 md:mt-5 md:block md:space-y-3 md:text-sm"><p dir="ltr" className="flex gap-3"><Phone size={16} className="text-accent" />{WHATSAPP_DISPLAY_NUMBER}</p><a dir="ltr" href={INSTAGRAM_URL} target="_blank" rel="noreferrer" className="flex gap-3 transition-colors hover:text-white" aria-label="Instagram Casa de Perla"><Instagram size={16} className="text-accent" />{INSTAGRAM_HANDLE}</a></div></div>
    </div>
